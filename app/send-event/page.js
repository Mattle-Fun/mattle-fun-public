...
import { getRanking } from "@/app/monke-event/server";
dayjs.extend(utc);
...
const Ranking = () => {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["send-ranking"],
    queryFn: () =>
      getRanking(
        UserRoles.userSend,
        dayjs.utc("2025-07-04").startOf("day").toDate(),
        dayjs.utc("2025-07-18").endOf("day").toDate(),
      ),
  });
  ...
  return (
    <div className={"flex flex-col"}>
      ...
      >
        <EventButton
          as={"a"}
          href={"https://x.com/mattlefun/status/1941130101179163038"}
          target={"_blank"}
        >
          <PixelTextBox
            $borderColor={"493B0C"}
        ...
        <div className={"flex flex-row gap-[20px] mt-[12px] mb-[28px]"}>
            ...
            >
              ...
              <StyledPopover
                content={
                  ...
                  >
                    <span className={"text-[#F1C315]"}>$150</span> in $SEND -
                    sent after campaign,
                    <br />
                    <span className={"text-[#F1C315]"}>$150</span> in $MATTLE -
                    sent post TGE
                  </div>
                }
              >
        ...
        <div className={"flex flex-row gap-[16px] mb-[15px]"}>
          {new Array(8).fill(0).map((_, index) => (
            <Image
              key={index}
              src={`/images/send${index}.png`}
              alt={"Send"}
              width={48}
              height={48}
            />
